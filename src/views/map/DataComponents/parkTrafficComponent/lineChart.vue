            ...
            areaStyle: {
              normal: {
                color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [{
                  offset: 0,
                  color: item[0].areaStyle[0]
                }, {
                  offset: 0.8,
                  color: item[0].areaStyle[1]
                }], false)
              }
            },
            itemStyle: {
    ...
    initChart (data = {}) {
      ...
      const defaultTooltip = {
        ...
        formatter: params => {
          ...
          let date = `<span style="color:#fff">${params.name}</span><br/>`
          let content = `<span style="display:inline-block;margin-right:5px;border-radius:10px;width:10px;height:10px;background-color:${params.color.colorStops[0].color};"></span> 
            ...
            <span style="color:${params.color.colorStops[0].color};">${data.value}</span>`
          return date + content
        },
