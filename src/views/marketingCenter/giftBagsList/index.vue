      ...
      <el-row>
        <el-col :span="20">
          ...
          >
            ...
            <el-form-item label="奖券标题">
              <el-input
                v-model.trim="searchForm.awardTitle"
                placeholder="请输入奖券标题"
                clearable
                @change="initSearch"
              />
            ...
            <el-form-item label="商家昵称">
              <el-input
                v-model.trim="searchForm.merchantNickName"
                placeholder="请输入商家昵称"
                clearable
                @change="initSearch"
              />
            ...
            <el-form-item label="奖券状态">
              <el-select
                v-model="searchForm.ruleStatus"
                placeholder="请选择奖券状态"
                clearable
                @change="initSearch"
              >
                <el-option
                  v-for="item in giftBagStatusOptions"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value"
                />
        ...
        >
          ...
          >
            <template slot-scope="scope">
              <p :class="['class-' + scope.row.ruleStatus]">
                {{ getLable(scope.row.ruleStatus, giftBagStatusOptions) }}
              </p>
            </template>
          </el-table-column>
          <el-table-column
...
import { getPCRuleList } from '@/api/admin'
import { giftBagStatusOptions } from '@/utils/lookup'
...
export default {
  ...
  methods: {
    ...
    getData () {
      ...
      getPCRuleList(this.searchForm).then(res => {
        const data = res.body || {}
